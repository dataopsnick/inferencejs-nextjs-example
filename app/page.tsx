...
const checkItemCrossedMiddle = (prediction: Prediction, frameWidth: number) => {
  ...
  return false;
};.scanned && !itemState.disarmed && centerX < disarmRegion) {
    itemState.disarmed = true;
    console.log(`🔵 DISARMED: ${itemId} at position ${centerX.toFixed(0)} (disarm: ${disarmRegion.toFixed(0)})`);
  }
  ...
  if (itemState.disarmed) {
    ...
    if (itemState.cleanupCounter > cleanupInterval) {
      ...
      console.log(`🗑️ REMOVED: ${itemId} (cleanup counter: ${itemState.cleanupCounter})`);
      delete previousPredictionsRef.current[itemId];
    }
  }
  
  // Reset disarmed state if object moves back to the right of the arm line
  if (itemState.disarmed && centerX > armRegion) {
    console.log(`♻️ RESET: ${itemId} moved back to start position`);
    itemState.armed = false;
    itemState.scanned = false;
    itemState.disarmed = false;
